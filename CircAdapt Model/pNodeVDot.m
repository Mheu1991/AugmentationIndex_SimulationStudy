...
% Artven-related cavities, Resistance ~ 1/Volume^2 ;(r^4) poiseuille
% weerstand neemt af bij grotere r
...
dpTb = dpWf(pTbNd,pTbE,pTbS,YTbS,YTbNd);
DqTb = -YTbS.*dpTb;
...
% dy= fraction of conductivity decrease, -1<dy<0
% dp= decrease of source pressure to simulate waterfall
%=====================
eps=0.2;
p=max(0,min(pE,pS)-pN);
y=YN./hypot(YN-YS,eps*YS);
dp= p.*y;
end

% function dy1=dy(pN,pE,pS,Y,YN)
% % {pN,pE,pS}= pressure {Node, External, Source}
% % {Y,YN}= conductivity {waterfall, Node total}
% % dy= fraction of conductivity decrease, -1<dy<0
% %=====================
% x=max(0,min(0.99,(pE-pN)./max(eps*abs(pE-pN),pS-pN)));
% dy1= - x;
% end
...
% % dy= fraction of conductivity decrease, -1<dy<0
% %=====================
% eps=0.2; %avoid zero division, smooth singularity
% x=max(0,min(1,(pE-pN)./max(eps*abs(pE-pN),pS-pN)));
% y= Y./YN ;
% dy1= - x ./( 1 - (1 - x)*(1 - eps) .* y );
% end

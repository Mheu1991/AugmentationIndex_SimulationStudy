...
Net.ArtVen.Name = {'Ca','Subcl','Axill','Bra','UlRad','Fi','Ce','Fe','Pu'}; % carotid, brachial, celiac, femoral + Pulmonary, 3 AVs at arm, finger %+++%
...
Net.Tube.Nodes=[... % proximal and distal node of elastic 
...
%+++%
Net.Bag.Name   ={'Peri','Thorax','PressureCuff'}; % pericardium, thorax, pressurecuff
%+++%
Net.Bag.Chamber={{'La','Ra'},{} ,{}     }; % enclosed chambers
Net.Bag.TriSeg ={{'v'}      ,{},{}      }; % enclosed TriSeg
Net.Bag.ArtVen ={{}         ,{'Pu'},{}  }; % enclosed ArtVen
%+++%
Net.Bag.Tube   ={{}         ,{'AoSubclAr','SubclArCeAr','VcSubclVe','SubclVeCeVe'}, {'AxillArCuffAr','AxillVeCuffVe'}}; %+++% Apply arm cuff and finger cuff
%+++%
Net.Bag.Bag    ={{}         ,{'Peri'} , {}}; % pericardium inside thorax

Net.Node.Baro ={'SubclAr'}; % pressure control node
...
P.General.AdaptFeedback    = 0.5;  % 0: NO PRESSURE CONTROL FEEDBACK!  
...
Put('Patch','Depolarization'  ,'All' , [0;0]-P.General.Dt/2);
...
global P
Sy = {'Ca','Subcl','Axill','Bra','UlRad','Fi','Ce','Fe'}; %+++%
Pu = 'Pu';
q0 = P.General.q0;

%Armq0 = 0.12; %+++++++ 20% of total initial flow (for example 85 mL/s) to arm/brachial artery segments 

Put('ArtVen','q0AV' ,Sy,[0.15,0.11,0.07,0.02,0.015,0.005,0.47,0.16]*q0); %++++++% flow target value [0.154,0.05,0.03,0.02,0.015,0.001,0.57,0.16]Put('ArtVen','q0AV' ,Pu, q0);
%   peripheral flow distribution over ArtVen vessel beds
Put('ArtVen','q0AV' ,Pu, q0);
Put('ArtVen','kAV'  ,Sy, 1);
Put('ArtVen','kAV'  ,Pu, 2);
Put('ArtVen','p0'   ,Sy,[p0 ; pRa]); %+++ pressure 

Put('ArtVen','p0'   ,Pu,[pPu; pLa]);
%Put('ArtVen','k'    ,'All',[12;12]); %estimate %++[12;12]
Put('ArtVen','k'    ,{'Ca','Subcl','Axill','Bra','UlRad','Fi','Ce','Fe','Pu'},[12,12,12,12,12,12,16,16,12;12,12,12,12,12,12,16,16,12]); %+++%
Put('ArtVen','Len'  ,'All',6.0*P.ArtVen.q0AV.^(1/3)); %estimate
Put('ArtVen','Len'  ,{'Ca','Subcl','Axill','Bra','UlRad','Fi','Fe'},[0.14,0.3,0.06,0.12,0.10,0.05,0.5]); %++++++++% %arm (Left SubclAr) and leg are long {'Subcl','Axill','Bra','UlRad','Fi','Fe'} [0.3,0.12,0.25,0.25,0.05,0.5]
...
Aux      = Get('Tube','A0','BraArUlRadAr');              %+++ Set cross tube area of tube UlRad (ulnar and radial artery combined) to 2* A0
Put('Tube','A0',{'BraArUlRadAr','BraVeUlRadVe'},2*Aux);

% Sequence TubeNames:
Art= {'AoSubclAr','SubclArAxillAr','AxillArCuffAr','CuffArBraAr','BraArUlRadAr','UlRadArFiAr','SubclArCaAr','SubclArCeAr','CeArFeAr'}; %+++%
Ven= {'VcSubclVe','SubclVeAxillVe','AxillVeCuffVe','CuffVeBraVe','BraVeUlRadVe','UlRadVeFiVe','SubclVeCaVe','SubclVeCeVe','CeVeFeVe'}; %+++%
All=[Art,Ven];
p0 = P.General.p0; % arterial pressure
q0 = P.General.q0;
Aux= Get('ArtVen','p0','Subcl');
...
%++++% Kalsey 2002, Majumdar 2012 
%Subclavian artery       ~ 5 cm -> Approximation
%Axillary artery         ~12 cm -> Majumdar 2012 (corrected for lower mean height)
%Brachial artery         ~25 cm -> Kalsey 2002
%Ulnar/Radial artery     ~25 cm -> Reymond 2011
%Finger/ digital artery  ~ 5 cm -> Approximation

%Length Subclavian artery -> Finger artery = 74 cm

%++++%

%Put('Tube','k'      ,All, repmat([8,11,11,11,11,11,11,11,15],[1,2]) );
%Put('Tube','k'      ,All, repmat([8,11,14,14,14,14,11,16,16],[1,2]) );
Put('Tube','k'      ,All, repmat([8,14,14,18,18,18,16,16,20],[1,2]) );
Put('Tube','Len'    ,All, repmat([5,10,6,6,27,5,5,23,15]/100,[1,2]));  %+++% [7,12,23,15]/100,[1,2])
Put('Tube','TauAtt' ,All, 0.2*sqrt(2e4*P.Tube.A0./P.Tube.k)); % wave attenuation time constant
...
P.Bag.k     = [10,10,1];
P.Bag.pAdapt= [200,50,1]; % transmural bag pressure pressure ; pAdapt ~= average pressure
%Estmate heart volume
VWall = sum(Get('Patch','VWall','All'));
VCav  = sum(Get('Cavity','V',Heart));
VHeart= VCav+VWall;
P.Bag.VRef= [1.4*VHeart,2.7*VHeart,8.7e-06]; % thorax volume not really used
end

function ValveParameters

A=Get('Tube','A0','AoSubclAr'); % Aortic cross-section
